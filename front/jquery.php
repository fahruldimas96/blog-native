				...
				<!-- row -->
						...
						<div class="row">
							...
							<?php  
					  		...
					  		$query = "SELECT * FROM posting, kategori, user WHERE posting.ID_KAT=4 AND posting.ID_STATUS=1 AND kategori.ID_KAT=posting.ID_KAT AND user.ID_ADMIN=posting.ID_ADMIN ORDER BY ID_POST DESC LIMIT $offset, $dataPerPage"; // Query untuk menampilkan semua data siswa
		...
		include "koneksi/config.php";
        $query3   = "SELECT  COUNT(ID_POST) AS jumData FROM posting WHERE ID_KAT=4";
          ...
          if ($jumData > 5)
            {
              ...
              echo  "<a href='?page=front/jquery&p=".($noPage-1)."'><button class='primary-button-1 center-block'>Prev</button></a>";
              ...
              for($page = 1; $page <= $jumPage; $page++)
              {
                ...
                if ((($page >= $noPage - 3) && ($page <= $noPage + 3)) || ($page == 1) || ($page == $jumPage))
                {
                  ...
                  else echo "<a href='?page=front/jquery&p=".$page."'><button class='primary-button-1 center-block'>".$page."</button></a>";
                }
              }
              ...
              if ($noPage < $jumPage) 
              echo "<a href='?page=front/jquery&p=".($noPage+1)."'><button class='primary-button-1 center-block'>Next</button></a>";
            }
