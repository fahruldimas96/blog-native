				...
				<!-- row -->
						...
						<div class="row">
							...
							<?php  
					  		...
					  		$query = "SELECT * FROM posting, kategori, user WHERE posting.ID_KAT=5 AND posting.ID_STATUS=1 AND kategori.ID_KAT=posting.ID_KAT AND user.ID_ADMIN=posting.ID_ADMIN ORDER BY ID_POST DESC LIMIT $offset, $dataPerPage"; // Query untuk menampilkan semua data siswa
							...
							<div class="col-md-12">
								<div class="post post-row">
									...
									<div class="post-body">
										<div class="post-meta">
											<a class="<?php echo ''.$data['NOTE'].''?>" href="?page=front/read&id=<?php echo ''.$data['ID_POST'].''?>">HOT</a>
										...
										<p><?php $namap=$data['TITLE_POST']; $potong = substr($namap,0,150); echo $potong;?></p>
									</div>
								</div>
							</div>
							<!-- /post -->
		...
		include "koneksi/config.php";
        $query3   = "SELECT  COUNT(ID_POST) AS jumData FROM posting WHERE ID_KAT=5";
          ...
          if ($jumData > 5)
            {
              ...
              echo  "<a href='?page=front/hot&p=".($noPage-1)."'><button class='primary-button-1 center-block'>Prev</button></a>";
              ...
              for($page = 1; $page <= $jumPage; $page++)
              {
                ...
                if ((($page >= $noPage - 3) && ($page <= $noPage + 3)) || ($page == 1) || ($page == $jumPage))
                {
                  ...
                  else echo "<a href='?page=front/hot&p=".$page."'><button class='primary-button-1 center-block'>".$page."</button></a>";
                }
              }
              ...
              if ($noPage < $jumPage) 
              echo "<a href='?page=front/hot&p=".($noPage+1)."'><button class='primary-button-1 center-block'>Next</button></a>";
            }
